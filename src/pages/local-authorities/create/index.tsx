...
import { createLocalAuthority } from 'apiSdk/local-authorities';
...
function LocalAuthorityCreatePage() {
  ...
  const handleSubmit = async (values: LocalAuthorityInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createLocalAuthority(values);
      resetForm();
  ...
  const formik = useFormik<LocalAuthorityInterface>({
    initialValues: {
      name: '',
      contact: '',
      city_id: (router.query.city_id as string) ?? null,
    },
