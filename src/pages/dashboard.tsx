...
import { getCrimeOrganisations } from 'apiSdk/crime-organisations';
import { CrimeOrganisationInterface } from 'interfaces/crime-organisation';
import { getLocalAuthorities } from 'apiSdk/local-authorities';
...
import { PersonListPage as ShowCaseList } from 'pages/people';

function DashboardPage() {
  const { session } = useSession();

  const crimeOrganisationStat = useSWR<PaginatedInterface<CrimeOrganisationInterface>>(
    () => `/crime-organisations`,
    () => getCrimeOrganisations({ limit: 1, offset: 0 }),
  );

  const localAuthorityStat = useSWR<PaginatedInterface<LocalAuthorityInterface>>(
    () => `/local-authorities`,
    () => getLocalAuthorities({ limit: 1, offset: 0 }),
  );

  const cityStat = useSWR<PaginatedInterface<CityInterface>>(
    () => `/cities`,
    () => getCities({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Crime Organisations',
      result: crimeOrganisationStat?.data?.totalCount,
      isLoading: crimeOrganisationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Local Authorities',
      result: localAuthorityStat?.data?.totalCount,
      isLoading: localAuthorityStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Cities',
      result: cityStat?.data?.totalCount,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`people`} passHref legacyBehavior>
                  <Button
