...
type ColumnType = ColumnDef<PersonInterface, unknown>;
...
export function PersonListPage(props: PersonListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getPeople({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'role.contains', 'bio.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PersonInterface>>(
  ...
  const handleView = (row: PersonInterface) => {
