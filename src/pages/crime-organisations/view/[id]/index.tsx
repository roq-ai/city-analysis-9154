...
import { getCrimeOrganisationById } from 'apiSdk/crime-organisations';
import { CrimeOrganisationInterface } from 'interfaces/crime-organisation';

function CrimeOrganisationViewPage() {
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Crime Organisation Details',
              isCurrent: true,
            },
          ]}
        />
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                <Box>
                  ...
                  >
                    Crime Organisation Details
                  </Text>
                </Box>
                {hasAccess('crime_organisation', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/crime-organisations/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('city', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="City"
                    text={
                      <Link as={NextLink} href={`/cities/view/${data?.city?.id}`}>
                        {data?.city?.name}
                      </Link>
                    }
                  />
